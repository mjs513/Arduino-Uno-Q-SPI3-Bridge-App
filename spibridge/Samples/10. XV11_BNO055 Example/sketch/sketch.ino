...
#define SerialX Serial3  //Lidar connection need @KurtE's mod
...
/***************************************************************************/
/******************  Configuration setup for a BNO55   *********************/
/***************************************************************************/
...
#define bno055_priority 7
//Thread:  SPI BRIDGE Frame thread Thread to be executed
void bno055(void)
{
    ...
    while (1) {
      ...
      imu_bno055.yaw = (float)event.orientation.x;
      imu_bno055.roll = -(float)event.orientation.y;
      imu_bno055.pitch = -(float)event.orientation.z;
...
K_THREAD_DEFINE(bno055_id, STACKSIZE, bno055, NULL, NULL, NULL, bno055_priority, 0, 0);
/************************************************************************************
 *     Configure SPI Bridge Thread including 
 *     Frame control
*************************************************************************************/
...
void spibridge(void)
{
    Serial.println(" spibridge Started\n");
        ...
        //if (buffer[0] == 0x0A) {
            switch (buffer[0]) {
                ...
                case 0x2A:   // IMU request
                    sendFrame(DATA_IMU, (uint8_t*)&imu_bno055, sizeof(imu_bno055));
                    break;
...
void setup() {
  ...
  analogWriteFrequency(xv_config.motor_pwm_pin, 4000);
  delay(100);
              ...
              for (int ix = 0; ix < N_DATA_QUADS; ix++) {
                  if (xv_config.aryAngles[startingAngle + ix]) {             // if we're supposed to display that angle
                      if (aryInvalidDataFlag[ix] & BAD_DATA_MASK) {  // if LIDAR reported a data error...
                          ...
                          //Serial.print(F(","));
                          flag[startingAngle + ix] = 0;
                          if (aryInvalidDataFlag[ix] & INVALID_DATA_FLAG){
                              flag[startingAngle + ix] = 1;
                              //Serial.println(F("I"));
                          }
                          if (aryInvalidDataFlag[ix] & STRENGTH_WARNING_FLAG){
                              flag[startingAngle + ix] = 2;
                              //Serial.println(F("S"));
                          }
                          angle[startingAngle + ix] = startingAngle + ix;
                          //}
                      }
                      else {                                         // show clean data
                          angle[startingAngle + ix] = startingAngle + ix;
                          range[startingAngle + ix] = int(aryDist[ix]);
                          quality[startingAngle + ix] = aryQuality[ix];
                      }
                  }
              }  // for (int ix = 0; ix < N_DATA_QUADS; ix++)
...
void getRange(){
  ...
  */
  for (int i = 0; i < 361; i++) {
    lidar.angle[i] = angle[i];
    lidar.flag[i] = flag[i];
    lidar.quality[i] = quality[i];
    lidar.range[i] =  range[i];
  }
}
